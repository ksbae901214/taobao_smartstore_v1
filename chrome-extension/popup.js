...
function extractBasicProductInfo() {
  ...
  try {
    ...
    for (const script of scripts) {
      ...
      // SKU ID 찾기 (10자리 이상 숫자)
      const allSkuIds = new Set();
      const skuIdPattern = /"(\d{10,})"\s*:\s*\{/g;
      let idMatch;
      ...
      // 패턴: {"propPath":"-1:-1","skuId":"6121264615356"} 또는 {"propPath":"1627207:23094317651","skuId":"5687256985173"}
      // vid는 음수(-1, -2 등) 또는 양수 가능!
      ...
      while ((mapMatch = skuMappingPattern.exec(text)) !== null) {
        ...
        // propPath: "-1:-1" 또는 "1627207:23094317651" 또는 여러 개 조합
        const parts = propPath.split(';');
        parts.forEach(part => {
          const colonIdx = part.lastIndexOf(':');
          if (colonIdx > 0) {
            const vid = part.substring(colonIdx + 1);  // "-1" 또는 "23094317651"
      ...
      // 패턴1: vid(음수 포함), image, name 순서
      // vid가 "-1" 또는 "23094317651" 형태
      ...
      // 패턴2: comboPropertyValue, vid(음수 포함), image, name 순서
